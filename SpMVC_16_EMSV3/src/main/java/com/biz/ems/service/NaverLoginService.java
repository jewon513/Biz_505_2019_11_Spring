...
public class NaverLoginService {
	...
	public String oAuthLoginGet() {
		...
		/*
		 * spring 4.1에서 사용하는 URIQuery 문자열을 생성하는 class 이다.
		 * fromHttpURL() : 요청을 수행할 server 주소
		 * queryParam() : 변수=값 형태의 query 문자열 생성
		 * build(true) : 생성하는 문자열중에서 encoding이 필요한 부분이 있으면 encoding을 수행하라
		 * 5.1 이상에서는 별도로 encoding() method가 있다.
		 */
		apiURL = UriComponentsBuilder.fromHttpUrl(this.loginAPI_URL).queryParam("client_id", this.clientId)
